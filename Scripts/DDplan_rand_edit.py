...
if __name__=='__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hw:o:l:d:f:b:n:k:c:t:s:r:",
...
""" % (dt, numchan, blocklen, BW, fctr, args[0]))
    ...
    if write_dedisp:
        seed()
        rand_val = random()
        multiplier = 4.5 * rand_val + 0.5
        #rdDMs = [round((m.dDM * (4.5 * random() + 0.5)), sigfigs=2) for m in methods]
        rdDMs = [round(m.dDM * multiplier, sigfigs=2) for m in methods]
...
#        print("dDMs:%s  dsubDMs:%s  startDMs:%s  downsamps:%s  dmspercalls:%s  subcalls:%s"%(dDMs,dsubDMs,startDMs,downsamps,dmspercalls,subcalls))
       # else :
        ...
        print(dDMs, dsubDMs,startDMs,downsamps,dmspercalls,subcalls,rdDMs)
        
        with open(j_file_name, 'w') as f:
            f.write("%s\n"%repr(dDMs))
            f.write("%s\n"%repr(dsubDMs))
            f.write("%s\n"%repr(startDMs))
            f.write("%s\n"%repr(downsamps))
