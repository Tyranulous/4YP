function plotter_parent(data,predicted,holdback_datain,mode)
%mode:  1 histogram_plotter
%       2 best versus 271
%       3 inter_tele_variance (samps) - what a horid name
%       4 inter_tele_variance (no samps)
%       5 Percent comparison
...
if exist('mode','var')
    switch mode
        ...
        case 3
            inter_tele_variance(data,1); %compares percent differnece between best and worst data point in each telescope
        case 4
            inter_tele_variance(data,0);
        case 5
            percent_comparison(predicted,holdback_datain);
        case 6
            dmplanplotter(data);
    end
else
...
%     timespersample = data(id).corrected_time(:)./data(id).samples;
    timespersample = data(id).corrected_time(:)./sum(data(id).dmplan(4,:)./data(id).dmplan(3,:));
    %find best time
    [best_times(id),time_locs(id)] = min(timespersample);
    hash271_times(id) = timespersample(271);
...
hash_labels2 = hash_labels(end-36:end);
temp_paper_thing = unique_hashdefines(end-36:end,5);
% bar(hash_labels,unique_hashdefines(:,5))
figure();
bar(hash_labels2,temp_paper_thing)
grid on
...
function percent_comparison(predicted,data)

best_times = zeros(length(data),1);
time_locs = zeros(length(data),1);
hash271_times = zeros(length(data),1);
predicted_times = zeros(length(data),1);

pred_hashdefines = zeros(length(data),1);
hashdefines = zeros(length(data),4);
cache_or_shared = zeros(length(data),1);
cfs = zeros(length(data),1);
%pct_diffs = zeros(length(data,1);
...
for id = 1:length(data)
    %check what format predicted takes
    %predicted_min_loc = 
    if size(predicted,1) > 1   
        t=id-1;
        [~,predicted_min_loc] = min(predicted((t*275+1):(id*275)));
    else
        [~,predicted_min_loc] = min(predicted(:,id));
    end
    %tempory time vector from datain
    ...
    %predicted
    predicted_times(id) = times(predicted_min_loc);
    pred_hashdefines(id,1:4) = data(id).time_logs(predicted_min_loc,2:5);
...
pct_diffs1 = [cfs,100 * ((hash271_times - best_times) ./ best_times)];
pct_diffs2 = [cfs,100 * ((predicted_times - best_times) ./ best_times)];

% shared_pct = pct_diffs1((cache_or_shared(:)==1),:);
% cache_pct = pct_diffs1((cache_or_shared(:)==2),:);
...
hold on
%scatter(cfs,pct_diffs)
%text(cfs + 3, pct_diffs1(:,2) + 3, big_hash_labels)
scatter(pct_diffs1(:,1),pct_diffs1(:,2),'filled');
scatter(pct_diffs2(:,1),pct_diffs2(:,2));
legend('Between best and 4-14-8-64','Between best and predicted')
xlabel('Central Freq')
ylabel('% Difference in Dedispersion time')

%to do, track cache or not
end
