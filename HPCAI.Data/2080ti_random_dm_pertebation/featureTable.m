...
for t = 1:telescopes
    ...
    %new feature: mean dm step over binning
     %features_matrix(locs,14) = dmplanstepmean(dmplan,dmplan_shape);
    [features_matrix(locs,14),features_matrix(locs,15)] = dmplan_line_fit(dmplan);
%      features_matrix(locs,15) = dmplan_variancer(dmplan);
...
tableOut = array2table([features_matrix,nan_time4],'VariableNames',{'numreg','divint','divindm','cf','tsamp','nchans','bw','max_dm_step','min_dm_step','max_dm','ndm_ranges','num_dm_samples','mean_dm_binning','mean_dm_step','time_per_sample'});
