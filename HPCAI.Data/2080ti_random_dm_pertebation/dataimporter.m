%dataimporter
clear datain greaterthan275 gtindex j more_datain
addpath('T:\OneDrive\Documents\Work\Machine Learning\4YP\HPCAI.Data\shared_scripts')
folders = dir('./');
...
for i = 3:(length(folders)-2)
    if folders(i).isdir == 1% && not(strcmp('cf-290__bw-100__tsamp-64__chans-4096',folders(i).name))
        ...
        try
            ...
            warning('fisrt telescope imported was a dud');
        end
        
        datain(index).time_logs = time_log_importer(path);
        
        ddplan_file_path = strcat(path,'\ddplan.dat');
        try
            if isfile(ddplan_file_path)
                datain(index).dmplan = ddplan_reader(path);
            else
                for j = 0:9
                    datain(index).dmplan{j+1} = ddplan_reader(path,j);
                end
            end
        catch
            error("DDplan probably doesn't exist")
        end
%         try
%             datain(index).dmplan = ddplan_reader(path);
%         catch
...
%         end
        ...
        datain(index).central_freq = datain(index).time_logs(1,6);
...
%        [~,datain(index).speedup] = txt_importer(path);
        ...
        if size(datain(index).time_logs,1) ~= 275 || iscell(datain(index).dmplan)
            if size(datain(index).time_logs,1) < 275
                ...
                if isempty(greaterthan275(1).time_logs)
                    greaterthan275(1) = datain(index);
                    greaterthan275(1).inputs = inputs_reader(path);
                else
                    gtindex = length(greaterthan275)+1;
                    greaterthan275(gtindex) = datain(index);
                    greaterthan275(gtindex).inputs = inputs_reader(path);
                end
            end
             datain(index) = [];
        end
...
more_datain = multiple_extractor(greaterthan275);
datain(end+1:end+length(more_datain)) = more_datain;

index = 1;
contained_zeros = 0;
for i = 1:length(datain)
    datain(index).containsZero = sum(datain(index).samples == 0);
    datain(index).samples(datain(index).samples == 0) = ...
        min(datain(index).samples(datain(index).samples>0));
    if datain(index).containsZero > 0
        datain(index) = [];
        contained_zeros = contained_zeros + 1;
    else
        index = index + 1;
    end
...
sorted_datain = datain;
%todo: sort datain by cf
for i = 1:length(datain)
    temp_min_cf = datain(1).central_freq;
    temp_min_location = 1;
    for ii = 1:length(datain)
        if temp_min_cf > datain(ii).central_freq
            temp_min_cf = datain(ii).central_freq;
            temp_min_location = ii;
        end
    end
    sorted_datain(i) = datain(temp_min_location);
    datain(temp_min_location) = [];
end
datain = sorted_datain;
[datain,rejectedDin] = monotonicDmFinder(datain);
datain = failsafe_calculator(datain);
% datain = monotonicDatain;
%clean up

for t = 1:length(datain)
    [~,i] = min(datain(t).corrected_time);
    datain(t).bestHashLoc = i;
end

for t = 1:length(datain)
    if datain(t).dmplan(3,1) ~= 1
        while true
            datain(t).dmplan(3,:) = datain(t).dmplan(3,:) ./ 2;
            if datain(t).dmplan(3,1) == 1
                break
            end
        end
    end
    datain(t).dmplan(4,:) = datain(t).dmplan(5,:) .* datain(t).dmplan(4,:) ...
                            ./ datain(t).dmplan(1,:);
end
datain1dm = only1dmrange(datain);

clear index i bwstarti bw temp_timestep temp_best_finder path min_loc best_time bwendi folders sorted_datain temp_min_cf temp_min_location ii gtindex j ddplan_file_path
cleanupforme
