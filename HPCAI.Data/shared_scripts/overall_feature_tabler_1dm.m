function [table, current_num_features] = overall_feature_tabler_1dm(data)

num_telescopes = length(data);
current_num_features = 10;
...
%     matrix(tlocs,1) = 
    ...
    matrix(tlocs,9) = dmplan(1);
    matrix(tlocs,10) = dmplan(4);
...
%     
%}
    matrix(tlocs,11) = normalize(data(t).corrected_time(:) ./ ...
        data(t).samples(:));
end
...
var_names = {'numreg','divint','divindm','failsafe','time_sampling','nchans','bw','cf','dmstep','dmtrials','y'};
