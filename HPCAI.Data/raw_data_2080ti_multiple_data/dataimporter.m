        ...
        if size(datain(index).time_logs,1) ~= 275 || iscell(datain(index).dmplan)
            if size(datain(index).time_logs,1) < 275
                ...
                if isempty(greaterthan275(1).time_logs)
                    ...
                    greaterthan275(length(greaterthan275)+1) = datain(index);
                end
            end
             datain(index) = [];
        end
...
%clean up
clear index i bwstarti bw temp_timestep temp_best_finder path min_loc best_time bwendi folders sorted_datain temp_min_cf temp_min_location ii
